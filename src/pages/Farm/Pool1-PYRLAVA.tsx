...
//
import TokenImg_p3 from '../../assets/images/PYRLAVA-POOL3.jpg'
...
const PYRLAVA_P1 = () => {
    ...
    const PYRLAVA_p2 = '0x96Dc2f81f8B7DE3CBc67BDa215c83F2881B440d6'
    const PYRLAVA_p3 = '0x5082b69c285af907208fd477F975192D7EBF517F'

    
    let pyrLavaPool1 = usePYRLAVA_POOL1_Contract(PYRLAVA_p1)
    let pyrLavaPool2 = usePYRLAVA_POOL1_Contract(PYRLAVA_p2)
    let pyrLavaPool3 = usePYRLAVA_POOL1_Contract(PYRLAVA_p3)
    ...
    const [EndJoinDate_LAVA_P2, setEndJoinDate_PYRLAVA_POOL2] = useState('');
    const [EndDate_LAVA_P2, setEndDate_PYRLAVA_POOL2] = useState('');
    const [totalPoolParticipant_LAVA_P2, setTotalPoolParticipant_PYRLAVA_POOL2] = useState('');
    const [minAmountToLock_LAVA_P2, setminAmountToLock_PYRLAVA_POOL2] = useState('');
    const [CurrentParticipants_LAVA_P2, getCurrentParticipants_PYRLAVA_POOL2] = useState('');


    const [EndJoinDate_LAVA_P3, setEndJoinDate_PYRLAVA_POOL3] = useState('');
    const [EndDate_LAVA_P3, setEndDate_PYRLAVA_POOL3] = useState('');
    const [totalPoolParticipant_LAVA_P3, setTotalPoolParticipant_PYRLAVA_POOL3] = useState('');
    const [minAmountToLock_LAVA_P3, setminAmountToLock_PYRLAVA_POOL3] = useState('');
    const [CurrentParticipants_LAVA_P3, getCurrentParticipants_PYRLAVA_POOL3] = useState('');
    ...
    useEffect(
        () => {
               ...
               pyrLavaPool1?.totalPoolParticipant()
                ...
                //2nd pool
                pyrLavaPool2?.totalPoolParticipant()
                .then((result2: any) => {
                    setTotalPoolParticipant_PYRLAVA_POOL2(result2)
                })




                pyrLavaPool2?.endJoin_date()
                .then((result1: any) => {
                    setEndJoinDate_PYRLAVA_POOL2(result1)
                })

                pyrLavaPool2?.unlock_date()
                .then((result1: any) => {
                    setEndDate_PYRLAVA_POOL2(result1)
                })


                
                pyrLavaPool2?.AmountToLock()
                .then((result1: any) => {
                    setminAmountToLock_PYRLAVA_POOL2(result1)
                })

                pyrLavaPool2?.getCntLocksForToken()
                .then((result1: any) => {
                    getCurrentParticipants_PYRLAVA_POOL2(result1)
                })
                ...
                pyrLavaPool3?.totalPoolParticipant()
                .then((result2: any) => {
                    setTotalPoolParticipant_PYRLAVA_POOL3(result2)
                })




                pyrLavaPool3?.endJoin_date()
                .then((result1: any) => {
                    setEndJoinDate_PYRLAVA_POOL3(result1)
                })

                pyrLavaPool3?.unlock_date()
                .then((result1: any) => {
                    setEndDate_PYRLAVA_POOL3(result1)
                })


                
                pyrLavaPool3?.AmountToLock()
                .then((result1: any) => {
                    setminAmountToLock_PYRLAVA_POOL3(result1)
                })

                pyrLavaPool3?.getCntLocksForToken()
                .then((result1: any) => {
                    getCurrentParticipants_PYRLAVA_POOL3(result1)
                })
    ...
    date_p2.setUTCSeconds(Number(EndJoinDate_LAVA_P2));

    var endDate_p2 = new Date(0); 
    endDate_p2.setUTCSeconds(Number(EndDate_LAVA_P2));
    ...
    date_p3.setUTCSeconds(Number(EndJoinDate_LAVA_P3));

    var endDate_p3 = new Date(0); 
    endDate_p3.setUTCSeconds(Number(EndDate_LAVA_P3));
    ...
    function poolStatus_p2() {
        ...
        if(totalPoolParticipant_LAVA_P2.toString()==CurrentParticipants_LAVA_P2.toString()){return "Closed" }
        else if (currentStamp_p2 < EndJoinDate_LAVA_P2) { return "Open" }
       else { return "Closed" }
   }
   ...
   function poolStatus_p3() {
    ...
    if(totalPoolParticipant_LAVA_P3.toString()==CurrentParticipants_LAVA_P3.toString()){return "Closed" }
    else if (currentStamp_p3 < EndJoinDate_LAVA_P3) { return "Open" }
   else { return "Closed" }
}
...
poolOpenStatus_p3 = poolStatus_p3()
    ...
    async function LOCK_P1_func() {
        ...
        if (formattedAmounts[Field.CURRENCY_B] != num) {
            ...
            const router = getPYR_P1_Contract(chainId, library,PYRLAVA_p1, account)
    ...
    async function LOCK_P2_func() {
        const num = toNonExponential(Number(minAmountToLock_LAVA_P2) / 10 ** 18)
        ...
        if (formattedAmounts[Field.CURRENCY_B] != num) {
            ...
            if (!chainId || !library || !account) return
            const router = getPYR_P1_Contract(chainId, library,PYRLAVA_p2, account)
            let estimate = router.estimateGas.lockLPTokenWithFixedTime
            let args: Array<string | string[] | number>;
            let method: (...args: any) => Promise<TransactionResponse>;
          ...
          //  alert("Lock called" + formattedAmounts[Field.CURRENCY_B])
            ...
            args = [
                minAmountToLock_LAVA_P2.toString(),
                account,
            ]
            value = null
    ...
    async function LOCK_P3_func() {
        const num = toNonExponential(Number(minAmountToLock_LAVA_P3) / 10 ** 18)
        ...
        if (formattedAmounts[Field.CURRENCY_B] != num) {
            ...
            if (!chainId || !library || !account) return
            const router = getPYR_P1_Contract(chainId, library,PYRLAVA_p3, account)
            let estimate = router.estimateGas.lockLPTokenWithFixedTime
            let args: Array<string | string[] | number>;
            let method: (...args: any) => Promise<TransactionResponse>;
          ...
          //  alert("Lock called" + formattedAmounts[Field.CURRENCY_B])
            ...
            args = [
                minAmountToLock_LAVA_P3.toString(),
                account,
            ]
            value = null
    ...
    const [approvalB_p2, approveBCallback_p2] = useApproveCallback(parsedAmounts[Field.CURRENCY_B], PYRLAVA_p2)

    const [approvalB_p3, approveBCallback_p3] = useApproveCallback(parsedAmounts[Field.CURRENCY_B], PYRLAVA_p3)
    return (
                                        ...
                                        <div className="Prizes p-4 h-100">
                                            <div className="row">
                                                <div className="col-12 col-lg-auto col-xl-6 col-xxl-6 mb-3 mb-xxl-0">
                                                    <img src={TokenImg_p3} alt={'image'} className={'img-fluid '} />
                                                </div>
                                                <div className="col">
                                                    <h4  style={{color: "#CE440E"}}>Hades Flag</h4>
                                                    <p className={'mb-0'}>
                                            ...
                                            (
                                                <div className="Prizes p-4 h-100">
                                                    <div className={'d-flex justify-content-between mb-3 pe-3'}>
                                                        ...
                                                        <span><span className={'OrangeColor'}>{toNonExponential(Number(minAmountToLock_LAVA_P3) / 10 ** 18).toString()}</span> Vulcan-V2</span>
                                                    </div>
                                    ...
                                    <div className={'col-xl-12 col-lg-12 col-md-12 mb-3 mt-4 mt-xxl-4 mb-xl-0'}>
                                        <div className={'row h-100'}>
                                            ...
                                            <div className={'col-xxl-4 col-xl-4 col-lg-6 col-md-4 col-sm-4 col-6 mb-3'}>
                                                ...
                                                <span className={'FSize_18 text-white'}>{(Number(totalPoolParticipant_LAVA_P3)).toString()}</span>

                                            </div>
                                            <div className={'col-xxl-4 col-xl-4 col-lg-6 col-md-4 col-sm-4 col-6 mb-3 '}>
                                                <label className={'d-block mb-1'}>Current Participants</label>
                                                <span className={'FSize_18 text-white'}>{CurrentParticipants_LAVA_P3.toString()}</span>
                                            </div>
                                            ...
                                            <p><a href={account!=null?`#/LockingDetails/PYR-LAVA/POOL-3/${PYRLAVA_p3}/${lp}`:'#/Farm'}  className={'OrangeColor text-decoration-on-hover'}>
                                ...
                                <div className={'row'}>
                                    <div className={'col-xxl col-xl-6 col-lg-12 col-md-12 mb-3 mb-xl-0'}>
                                        <div className="Prizes p-4 h-100">
                                            <div className="row">
                                                <div className="col-12 col-lg-auto col-xl-6 col-xxl-6 mb-3 mb-xxl-0">
                                                    <img src={'https://cdn.discordapp.com/attachments/882160540338782208/902190480408850472/Olive_Tunic_lvl_1.png'} alt={'image'} className={'img-fluid '} />
                                                </div>
                                                <div className="col">
                                                    <h4  style={{color: "#3B5331"}}>Olive Tunic</h4>
                                        ...
                                        {poolOpenStatus_p2 == 'Open' ?
                                            (
                                                <div className="Prizes p-4 h-100">
                                                    <div className={'d-flex justify-content-between mb-3 pe-3'}>
                                                        <p className="text-white FSize_16 mb-0">Lock Limit</p>
                                                        <span><span className={'OrangeColor'}>{toNonExponential(Number(minAmountToLock_LAVA_P2) / 10 ** 18).toString()}</span> Vulcan-V2</span>
                                                    </div>
                                    ...
                                    <div className={'col-xl-12 col-lg-12 col-md-12 mb-3 mt-4 mt-xxl-4 mb-xl-0'}>
                                        <div className={'row h-100'}>
                                            ...
                                            <div className={'col-xxl-4 col-xl-4 col-lg-6 col-md-4 col-sm-4 col-6 mb-3'}>
                                                <label className={'d-block mb-1'}>Max Participants</label>
                                                <span className={'FSize_18 text-white'}>{(Number(totalPoolParticipant_LAVA_P2)).toString()}</span>

                                            </div>
                                            <div className={'col-xxl-4 col-xl-4 col-lg-6 col-md-4 col-sm-4 col-6 mb-3 '}>
                                                <label className={'d-block mb-1'}>Current Participants</label>
                                                <span className={'FSize_18 text-white'}>{CurrentParticipants_LAVA_P2.toString()}</span>
                                            </div>
                                            <div className={'cool-xxl-4 col-xl-4 col-lg-6 col-md-4 col-sm-4 col-6 mb-3'}>
                                                <label className={'d-block mb-1'}>Status</label>
                                            ...
                                            <p><a href={account!=null?`#/LockingDetails/PYR-LAVA/POOL-2/${PYRLAVA_p2}/${lp}`:'#/Farm'}  className={'OrangeColor text-decoration-on-hover'}>
 ...
 {/*First Pool*/}
                                ...
                                <div className={'row'}>
                                    ...
                                    <div className={'col-xl-12 col-lg-12 col-md-12 mb-3 mt-4 mt-xxl-4 mb-xl-0'}>
                                        <div className={'row h-100'}>
                                            ...
                                            <div className={'col-xxl-4 col-xl-4 col-lg-6 col-md-4 col-sm-4 col-6 mb-3'}>
                                                <label className={'d-block mb-1'}>Max Participants</label>
                                                <span className={'FSize_18 text-white'}>{(Number(totalPoolParticipant_LAVA_P1)).toString()}</span>
                                            ...
                                            <p><a href={account!=null?`#/LockingDetails/PYR-LAVA/POOL-1/${PYRLAVA_p1}/${lp}`:'#/Farm'}  className={'OrangeColor text-decoration-on-hover'}>
                        ...
                        <div className={'col-sm-12 col-md-4 col-lg-4 col-xl-3 col-xxl-3'}>
                            <div className={'border-start border-color-grey pe-4 ps-5 py-4 text-center text-sm-start h-100'}>
                                ...
                                <p><a href={"https://vulcandex.vulcanforged.com/#/add/0xb4666B7402D287347DbBDC4EA5b30E80C376c0B3/0x348e62131fce2F4e0d5ead3Fe1719Bc039B380A9"} target="_blank" className={'OrangeColor text-decoration-on-hover'}>
                                    Get PYR-LAVA LP
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                         viewBox="0 0 24 24" fill="none" stroke="#EB7527"
                                         stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
